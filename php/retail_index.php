...
<head>
    ...
    margin: 0;
    padding: 0;
    background-image: url('R.jpg'); /* Add your background image path */
...
.success {
    animation: success-animation 1ms ease-in-out;
}
    </style>
</head>
<body>
                ...
                if(isset($_POST['submit'])){
                    ...
                    if(is_array($row) && !empty($row)){
                        if(password_verify($password, $row['Password'])) {
                            ...
                            $_SESSION['id'] = $row['Id'];
                            echo "<script>
                                      document.getElementById('success-animation').classList.add('success');
                                  </script>";
                            header("Location: jklastop12sfkfio4.php");
                            exit(); // Add this to stop further execution
                        } else {
                            echo "<div class='message'>
                                <p>Wrong Password</p>
                                </div> <br>";
                            echo "<a href='retail_index.php'><button class='btn'>Go Back</button>";
                        }
                    }else{
                        echo "<div class='message'>
                        <p>User not found</p>
